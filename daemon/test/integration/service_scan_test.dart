...
import 'package:cadence_media/cadence_media.dart'
    hide File, Directory, Link, FileSystemEntity;
...
void _copy(String fixture, String destination) =>
    File(fixture).copySync(destination);

/// A desk that keeps score — how many times the watcher knocked.
class _CountingCoordinator extends ScanCoordinator {
  _CountingCoordinator(super.db);

  int incrementalCalls = 0;

  @override
  Future<bool> startIncremental(int libraryId, {required Set<String> dirs}) {
    incrementalCalls++;
    return super.startIncremental(libraryId, dirs: dirs);
  }
}
...
void main() {
  group('service scan routes', () {
    ...
    setUp(() {
      ...
      temp = Directory.systemTemp.createTempSync('cadence_service_scan_');
    });
    ...
    test('the watch setting turns the lookout on and off', () async {
      final db = MediaDatabase(NativeDatabase.memory());
      final coordinator = ScanCoordinator(db);
      final watch = LocalLibraryWatchService(db, coordinator);
      final wired = MediaClient.direct(
        db,
        service: MediaService(db, coordinator: coordinator, watch: watch),
      );
      addTearDown(wired.close);

      expect(watch.running, isFalse);
      await wired.setSetting(ScannerSettings.watchFolders, true);
      expect(watch.running, isTrue);
      await wired.setSetting(ScannerSettings.watchFolders, false);
      expect(watch.running, isFalse);

      // Any other key leaves the lookout alone.
      final unrelated = SettingDef.json<num>('playback.crossfade_seconds', 2);
      await wired.setSetting(unrelated, 3);
      expect(watch.running, isFalse);
    });

    test('deleting a library lets its watched roots go', () async {
      final db = MediaDatabase(NativeDatabase.memory());
      final coordinator = _CountingCoordinator(db);
      final watch = LocalLibraryWatchService(
        db,
        coordinator,
      ...
      final libraryId = await wired.createLibrary('Music', LibraryType.music);
      await wired.addRoot(libraryId, temp.path);
      await wired.setSetting(ScannerSettings.watchFolders, true);
      expect(watch.running, isTrue);
      await Future<void>.delayed(const Duration(milliseconds: 400));

      await wired.deleteLibrary(libraryId);
      expect(watch.running, isTrue, reason: 'the lookout itself stays on');

      // A stir in the abandoned root reaches nobody.
      _copy(Fixtures.id3v23Mp3, p.join(temp.path, 'orphan.mp3'));
      await Future<void>.delayed(const Duration(milliseconds: 500));
      expect(coordinator.incrementalCalls, 0);
    });

    test('the embedded transport scans too', () async {
      final spawned = MediaClient.direct(
        MediaDatabase(NativeDatabase.memory()),
      );
      addTearDown(spawned.close);

      final dir = Directory(p.join(temp.path, 'spawned'))..createSync();
      _copy(Fixtures.id3v23Mp3, p.join(dir.path, 'song.mp3'));
      final libraryId = await spawned.createLibrary('Music', LibraryType.music);
      await spawned.addRoot(libraryId, dir.path);

      expect((await spawned.scan(libraryId)).status, 202);
      final status = await settle(spawned, libraryId);
      expect(status.state, ScanState.done);
      expect(status.added, 1);
      expect(await spawned.mediaItems(libraryId), hasLength(1));
    });
  });
}
