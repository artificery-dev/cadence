...
void main() {
  ...
  setUp(() async {
    ...
    root = Directory.systemTemp.createTempSync('cadence_scan');
  ...
  group('LibraryScanner', () {
    test('a first scan adds files, items, tags, and hashes', () async {
      ...
      expect(
        hashA.value,
        crypto.sha256.convert(File(a).readAsBytesSync()).toString(),
      );
    });
    ...
    test(
      ...
      () async {
        ...
        File(a).writeAsStringSync('aaa', flush: true);
        File(a).setLastModifiedSync(stat.modified);
    ...
    test('in an image library images are items', () async {
      ...
      final wallRoot = Directory.systemTemp.createTempSync('cadence_walls');
      addTearDown(() => wallRoot.deleteSync(recursive: true));
      await repo.addRoot(imagesId, wallRoot.path);
      File(
        p.join(wallRoot.path, 'photo.png'),
      ).writeAsStringSync('pixels', flush: true);
      File(
        p.join(wallRoot.path, 'track.mp3'),
  ...
  group('ScanCoordinator', () {
    ...
    test('scanAll queues every library and drains them', () async {
      ...
      final secondRoot = Directory.systemTemp.createTempSync('cadence_more');
      addTearDown(() => secondRoot.deleteSync(recursive: true));
      await repo.addRoot(secondId, secondRoot.path);
      File(
        p.join(secondRoot.path, 'b.mp3'),
  ...
  group('LibraryWatchService', () {
    test('a stirred folder scans itself after the quiet spell', () async {
      ...
      final lookout = LocalLibraryWatchService(
        db,
        desk,
        debounce: const Duration(milliseconds: 150),
      );
      await lookout.start();
      addTearDown(lookout.stop);
      expect(lookout.running, isTrue);
      // Let the watcher finish posting itself before anything moves.
      await Future<void>.delayed(const Duration(milliseconds: 400));

      File(
        p.join(root.path, 'fresh.mp3'),
      ).writeAsStringSync('new', flush: true);
      await _until(() async => (await items()).isNotEmpty);

      await lookout.stop();
      expect(lookout.running, isFalse);
    });

    test('a deleted library does not keep the lookout knocking', () async {
      final desk = _CountingCoordinator(db);
      final lookout = LocalLibraryWatchService(
        db,
        desk,
      ...
      await LibraryRepository(db).deleteLibrary(libraryId);
      File(
        p.join(root.path, 'orphan.mp3'),
      ).writeAsStringSync('aaa', flush: true);

      // One event, then silence: the debt is dropped, not re-armed.
      await Future<void>.delayed(const Duration(milliseconds: 500));
      final settled = desk.incrementalCalls;
      await Future<void>.delayed(const Duration(milliseconds: 500));
      expect(desk.incrementalCalls, settled);
    });
  });
}
